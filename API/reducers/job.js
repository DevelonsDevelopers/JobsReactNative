...
const job = (state = { isLoading: true, success: false, error: false, nodata: false }, action) => {
    switch (action.type) {
        ...
        case BOOKMARK_JOB:
            ...
            const recommendedJobs = state.recommendedJobs?.map((job) => (job.id === action.payload.job ? { ...job, bookmark: action.payload.bookmark } : job))
         

            return { ...state, jobs: jobs, cityJobs: cityJobs, companyJobs: companyJobs, categoryJobs: categoryJobs, recommendedJobs: recommendedJobs, }
        case RESET:
            return { ...state, isLoading: true, success: false, error: false, nodata: false }
        default:
            return state
