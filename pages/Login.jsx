import {
  ...
  import {
    LoginAuthentication,
    ProviderLoginAuthentication,
  } from "../API/actions/loginActions";
  ...
  function Login({ route, navigation }) {
    ...
    const seekerSuccess = useSelector(
      (state) => state.success.seekerLoginSuccess
    );
    const providerSuccess = useSelector(
      (state) => state.success.providerLoginSuccess
    );
    const seekerLoading = useSelector(
      (state) => state.loading.seekerLoginLoading
    );
    const providerLoading = useSelector(
      (state) => state.loading.providerLoginLoading
    );
    ...
    GoogleSignin.configure({
      webClientId:
        "404623696003-doe1cpjrlljj8om770ha3ri9s9vatoc8.apps.googleusercontent.com", // Replace with your actual Web Client ID
    });
    ...
    const handleGoogleSignIn = async () => {
      try {
        ...
        await google(
          user.name,
          user.givenName + user.id.substring(user.id.length - 6),
          user.email,
          "",
          "",
          "",
          ...
          .then(async (res) => {
            ...
            if (responseCode === 200) {
              ...
              if (data.affectedRows === 1) {
                toggleLoadingVisibility(false);
                navigation.popToTop();
                navigation.replace("GoogleRegister", {
                  id: ID,
                  name: user.name,
                  email: user.email,
                });
      ...
      try {
        ...
        await google(
          ...
          if (responseCode === 200) {
            ...
            sleep(2000).then(async () => {
              await AsyncStorage.setItem("LOGIN", "true");
              await AsyncStorage.setItem("ID", "9999");
              await AsyncStorage.setItem("USER", "SEEKER");
              await AsyncStorage.setItem("NAME", "Apple");
              await AsyncStorage.setItem("EMAIL", "apple@gmail.com");
              await AsyncStorage.setItem("USERNAME", "Apple0099");
              toggleLoadingVisibility(false);
              navigation.popToTop();
              navigation.replace("Home");
            });
    ...
    return (
      <View style={{ flex: 1 }}>
        ...
        >
          ...
          >
            <Pressable
              onPress={() => navigation.replace("Home", { Status: 0 })}
              style={{
                marginLeft: "auto",
                marginRight: 20,
          ...
          >
            ...
            >
              ...
              <Pressable
                onPress={() => navigation.replace("Register", { USER: USER })}
              >
